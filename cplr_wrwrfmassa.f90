...
!   end if
  ...
  ! Declare local variables
    ...
    character(6) filename
    integer(i_kind) i,j,k,kt,kq,ku,kv,kw,krefl,it,i_psfc,i_t,i_q,i_u,i_v,i_w,i_refl
    integer(i_kind) ii_ql,ii_qi,ii_qr,ii_qs,ii_qg   !i_qnr,i_qni,i_qnc
    integer(i_kind) kkql,kkqi,kkqr,kkqs,kkqg
    integer(i_kind) i_qc,i_qi,i_qr,i_qs,i_qg,i_qnr,i_qni,i_qnc
    integer(i_kind) kqc,kqi,kqr,kqs,kqg,kqnr,kqni,kqnc,i_tt,ktt
    ...
    real(r_kind), pointer :: ges_refl(:,:,:)=>NULL()
    real(r_kind), pointer :: ges_q (:,:,:)=>NULL()
    real(r_kind), pointer :: ges_ql(:,:,:)=>NULL() ! wangjia: I use ql instead of qc
    real(r_kind), pointer :: ges_qc(:,:,:)=>NULL()
    real(r_kind), pointer :: ges_qi(:,:,:)=>NULL()
    real(r_kind), pointer :: ges_qr(:,:,:)=>NULL()
    real(r_kind), pointer :: ges_qs(:,:,:)=>NULL()
...
! wangjia: U/V/T/Q/W/REFL, i_psfc/i_sst/i_skt
    num_mass_fields_base=2+4*lm + 1
    if(wangjia_luse_wwnd) num_mass_fields_base=num_mass_fields_base+lm ! wangjia: add "W"
    if(wangjia_luse_refl) num_mass_fields_base=num_mass_fields_base+6*lm ! wangjia: add "REFLectivity",ql,qr,qi,qs,qg
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
    ...
    i_u=i_q+lm
    i_v=i_u+lm
    i_w=-9999; i_refl=-9999
    ii_ql=-9999; ii_qr=-9999; ii_qi=-9999; ii_qs=-9999; ii_qg=-9999
...
! wangjia: sigf02->BK  on staggered grids
!          siganl->ANL on staggered grids
    if(mype == 0) then
  ...
  ! get pointer to relevant instance of cloud-related backgroud
    ...
    call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'u' , ges_u , istatus );ier=ier+istatus
    call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'v' , ges_v , istatus );ier=ier+istatus
    if(wangjia_luse_wwnd) call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'w' , ges_w , istatus );ier=ier+istatus
    if(wangjia_luse_refl) then
       call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'refl',ges_refl, istatus );ier=ier+istatus
    ...
    call GSI_BundleGetPointer ( GSI_MetGuess_Bundle(it), 'q' , ges_q , istatus );ier=ier+istatus
    if (ier/=0) then ! doesn't have to die - code can be generalized to bypass missing vars
        write(6,*)'wrwrfmassa_binary: getpointer failed, cannot retrieve ps,u,v,w,q'
        call stop2(999)
    endif
  ...
  ! for hydrometeors
       ...
       do i=1,lon2
          do j=1,lat2
             ...
             if(wangjia_luse_wwnd) all_loc(j,i,kw)=ges_w(j,i,k)
             if(wangjia_luse_refl) then
                all_loc(j,i,krefl)=ges_refl(j,i,k)
                all_loc(j,i,kkql) =ges_ql(j,i,k)
                all_loc(j,i,kkqr) =ges_qr(j,i,k)
                all_loc(j,i,kkqi) =ges_qi(j,i,k)
                all_loc(j,i,kkqs) =ges_qs(j,i,k)
                all_loc(j,i,kkqg) =ges_qg(j,i,k)
             end if
...
if(wangjia_luse_wwnd) then
    ...
    do k=1,nsig
       kw=kw+1
! wangjia: temp1 -> original values (maybe staggered)
       if(mype == 0) read(lendian_in)temp1
       call strip(all_loc(:,:,kw),strp)
! wangjia: tempa -> updated guess field
       call mpi_gatherv(strp,ijn(mype+1),mpi_real4, &
            tempa,ijn,displs_g,mpi_real4,0,mpi_comm_world,ierror)
       if(mype == 0) then 
! wangjia: tempb -> un-staggered and re-ordered field 
          call fill_mass_grid2t(temp1,im,jm,tempb,2)
! wangjia: tempa on lhs -> increments on mass grids
          do i=1,iglobal
             tempa(i)=tempa(i)-tempb(i)
          end do
          call unfill_mass_grid2t(tempa,im,jm,temp1)
    ...
    do k=1,nsig ! "sigf02" has "W", so still need to read it
       if(mype == 0) read(lendian_in)temp1
    end do
end if
...
if(wangjia_luse_refl) then
    ...
    do k=1,nsig ! "sigf02" has "REFL", so still need to read it
       if(mype == 0) read(lendian_in)temp1
    end do
end if
