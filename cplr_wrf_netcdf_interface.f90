...
use abstract_convert_netcdf_mod 
  ...
  contains
    ...
    procedure, pass(this) :: convert_netcdf_constraints => convert_netcdf_constraints_wrf
...
contains  
  ...
  !$$$
    ...
    use jfunc, only: wangjia_luse_wwnd,wangjia_luse_refl
...
! wangjia: add "field2u/field2v"
    real(r_single),allocatable::field2u(:,:),field2v(:,:)
...
! wangjia: if(abs(icrtmuse)>=10.and.abs(icrtmuse)<20) n3dcloud=n3dcloud+1 ! convention, for now
!          there are 2D clouds (clouds::2d) entry, but seems like there are NO actual 2D clouds
  ...
  ! to temporary binary format
    ...
    n_loop: do n=1,9 ! loop over forecast hours in assim interval
       ...
       if(n==nhr_assimilation)then ! wangjia: for now, nhr_assimilation=2
          flnm1 = 'wrf_inout'
       else
...
! wangjia: write WRF file from netCDF to binary
  ...
  !-------------  get grid info
       ...
       ! wangjia: add "field2u/field2v"
       allocate(field2u(nlon_regional+1,nlat_regional),field2v(nlon_regional,nlat_regional+1))
...
! wangjia: add W (vertical wind)
       ...
       do k=1,nsig_regional
          ...
          do i=1,nlon_regional
             ! average
             field3(i,j,k)=(field3(i,j,k)+field3(i,j,k+1))/2.0
          end do
          end do
          if(print_verbose) &
             write(6,*)' k,max,min,mid W=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
               field3(nlon_regional/2,nlat_regional/2,k)
          write(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! W
       end do
...
! wangjia: add reflectivity
       ...
       do k=1,nsig_regional
          if(print_verbose) &
             write(6,*)' k,max,min,mid REFL_10CM=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
               field3(nlon_regional/2,nlat_regional/2,k)
          write(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! REFL_10CM
       end do

if(wangjia_luse_refl) then
          rmse_var='QCLOUD'
          call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
               start_index,end_index, WrfType, ierr    )
          if(print_verbose)then
          ...
          call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
               ...
               ierr                                 )
          do k=1,nsig_regional
             if(print_verbose)write(6,*)' k,max,min,mid Ql=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &      
                      field3(nlon_regional/2,nlat_regional/2,k)
...
! wangjia: add "dx/mapfac_u" and "dy/mapfac_v"
       rmse_var='MAPFAC_U'
       call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
            start_index,end_index, WrfType, ierr    )
       if(print_verbose)then
       ...
       call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
            field2u,WRF_REAL,0,0,0,ordering,           &
            staggering, dimnames ,               &
            start_index,end_index,               & !dom
            start_index,end_index,               & !mem
            start_index,end_index,               & !pat
            ierr                                 )
       if(print_verbose)then
          write(6,*)' max,min mapfac_U=',maxval(field2u),minval(field2u)
          write(6,*)' max,min MAPFAC_U(:,1)=',maxval(field2u(:,1)),minval(field2u(:,1))
          write(6,*)' max,min MAPFAC_U(1,:)=',maxval(field2u(1,:)),minval(field2u(1,:))
          write(6,*)' mapfac_U(1,1),mapfac_U(nlon+1,1)=',field2u(1,1),field2u(nlon_regional+1,1)
          write(6,*)' mapfac_U(1,nlat),mapfac_U(nlon+1,nlat)=', &
            field2u(1,nlat_regional),field2u(nlon_regional+1,nlat_regional)
       end if
       field2u=one_single/(field2u*rdx)  !DX_MC
       write(6,*)' max,min DX_MC=',maxval(field2u),minval(field2u)
       write(iunit)field2u  ! dx/map (u-stagger)
       ...
       call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
            field2v,WRF_REAL,0,0,0,ordering,           &
            staggering, dimnames ,               &
            start_index,end_index,               & !dom
            start_index,end_index,               & !mem
            start_index,end_index,               & !pat
            ierr                                 )
       if(print_verbose)then
          write(6,*)' max,min mapfac_V=',maxval(field2v),minval(field2v)
          write(6,*)' max,min MAPFAC_V(:,1)=',maxval(field2v(:,1)),minval(field2v(:,1))
          write(6,*)' max,min MAPFAC_V(1,:)=',maxval(field2v(1,:)),minval(field2v(1,:))
          write(6,*)' mapfac_V(1,1),mapfac_V(nlon,1)=',field2v(1,1),field2v(nlon_regional,1)
          write(6,*)' mapfac_V(1,nlat+1),mapfac_V(nlon,nlat+1)=', &
            field2v(1,nlat_regional+1),field2v(nlon_regional,nlat_regional+1)
       end if
       field2v=one_single/(field2v*rdy)  !DY_MC
       write(6,*)' max,min DY_MC=',maxval(field2v),minval(field2v)
       write(iunit)field2v  ! dy/map (v-stagger)


       deallocate(field1,field1a,field2,field2b,field2c,ifield2,field3,field3u,field3v)
       deallocate(field2u,field2v)
       close(iunit)
  ...
  ! Declare local parameters
    ...
    logical print_verbose
    integer(i_kind) :: nums_neg,nums_tooHigh ! nums_tooHigh: # of pts > reflval_tooHigh
    real(r_single)  :: reflval_tooHigh = 80.0 ! [dBZ]
  ...
  !-------------  get grid info
    ...
    allocate(field3u(nlon_regional+1,nlat_regional,nsig_regional))
    allocate(field3v(nlon_regional,nlat_regional+1,nsig_regional))
    allocate(field3w(nlon_regional,nlat_regional,nsig_regional+1)) ! wangjia: to store fields on vertically staggered grids
  ...
  !        nlon_regional,nlat_regional,nsig_regional,pt_regional
    ...
    ! wangjia: check # of points with negative analysis
    nums_neg=0
    do k=1,nsig_regional
    do j=1,nlat_regional
    do i=1,nlon_regional
       if(field3(i,j,k)<0.0)             nums_neg    =nums_neg+1
    end do; end do; end do
    write(6,*) "****** ",trim(rmse_var),": # of Negative Val = ",nums_neg," (",1.0*nums_neg/(nsig_regional*nlat_regional*nlon_regional)*100.0,"%)" 
...
! wangjia: add "W"
if(wangjia_luse_wwnd) then
    do k=1,nsig_regional ! analysis on mass grid
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! W
       if(print_verbose) &
          write(6,*)' k,max,min,mid W=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    ...
    call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
         ...
         start_index,end_index1,               & !mem
         start_index,end_index1,               & !pat
         ierr                                 )     ! BK on staggered grid
    do k=1,nsig_regional ! increments on mass grid
       do j=1,nlat_regional
       do i=1,nlon_regional
          field3(i,j,k) = field3(i,j,k)-0.5*(field3w(i,j,k)+field3w(i,j,k+1))
       end do
       end do
    end do
    do k=1,nsig_regional+1 ! add increments back to BK on staggered grid
       km=max(1,k-1)
       k0=min(nsig_regional,k)
       do j=1,nlat_regional
       do i=1,nlon_regional
          field3w(i,j,k) = field3w(i,j,k)+0.5*(field3(i,j,k0)+field3(i,j,km))
       end do
       end do
    end do
    call ext_ncd_write_field(dh1,DateStr1,TRIM(rmse_var),              &
...
if(wangjia_luse_refl) then
    ...
    ! wangjia: check # of points with negative analysis
    nums_neg=0; nums_tooHigh=0
    do k=1,nsig_regional
    do j=1,nlat_regional
    do i=1,nlon_regional
       if(field3(i,j,k)<0.0)             nums_neg=nums_neg+1
       if(field3(i,j,k)>reflval_tooHigh) nums_tooHigh=nums_tooHigh+1
    end do; end do; end do
    write(6,*) "****** ",trim(rmse_var),": # of Negative Val = ",nums_neg," (",1.0*nums_neg/(nsig_regional*nlat_regional*nlon_regional)*100.0,"%)" 
    write(6,*) "****** ",trim(rmse_var),": # of Val(>",reflval_tooHigh,") = ",nums_tooHigh," (",1.0*nums_tooHigh/(nsig_regional*nlat_regional*nlon_regional)*100.0,"%)" 
    call ext_ncd_write_field(dh1,DateStr1,TRIM(rmse_var),              &
         field3,WRF_REAL,0,0,0,ordering,           &
         staggering, dimnames ,               &
         start_index,end_index1,               & !dom
    ...
    do k=1,nsig_regional
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! Q
       if(print_verbose) &
          write(6,*)' k,max,min,mid QCLOUD=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    rmse_var='QCLOUD'
    call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
    ...
    do k=1,nsig_regional
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! Q
       if(print_verbose) &
          write(6,*)' k,max,min,mid QRAIN=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    rmse_var='QRAIN'
    call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
    ...
    do k=1,nsig_regional
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! Q
       if(print_verbose) &
          write(6,*)' k,max,min,mid QICE=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    rmse_var='QICE'
    call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
    ...
    do k=1,nsig_regional
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! Q
       if(print_verbose) &
          write(6,*)' k,max,min,mid QSNOW=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    rmse_var='QSNOW'
    call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
    ...
    do k=1,nsig_regional
       read(iunit)((field3(i,j,k),i=1,nlon_regional),j=1,nlat_regional)   ! Q
       if(print_verbose) &
          write(6,*)' k,max,min,mid QGRAUP=',k,maxval(field3(:,:,k)),minval(field3(:,:,k)), &
            field3(nlon_regional/2,nlat_regional/2,k)
    end do
    rmse_var='QGRAUP'
    call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
...
! wangjia: add "convert_netcdf_constraints_wrf"
  subroutine convert_netcdf_constraints_wrf(this)
  ! use WRF library to read mass/moisture constraints, and write the result 
  ...
  ! to temporary binary format
    ...
    n_loop: do n=1,9 ! loop over forecast hours in assim interval
       ...
       if(n==nhr_assimilation)then
          ...
          write(flnm1,'("wrf_ad",i1.1)')n
       endif

       call ext_ncd_open_for_read( trim(flnm1), 0, 0, "", dh1, Status)
       if(n==nhr_assimilation)then
          if ( Status /= 0 )then
             write(6,*)'CONVERT_NETCDF_CONSTRAINTS:  problem with flnm1 = ',&
                  trim(flnm1),', Status = ', Status
             call stop2(74)
          endif
       else
  ...
  !-------------  get date info
       ...
       call ext_ncd_get_next_time(dh1, DateStr1, Status_next_time)
       read(DateStr1,'(i4,1x,i2,1x,i2,1x,i2,1x,i2,1x,i2)') iyear,imonth,iday,ihour,iminute,isecond
       write(6,*)'wangjia: constraints -> iy,m,d,h,m,s=',iyear,imonth,iday,ihour,iminute,isecond
  ...
  !-------------  get grid info
       ...
       nlon_regional=end_index(1)
       nlat_regional=end_index(2)
       if(print_verbose)write(6,*)' nlon,lat_regional=',nlon_regional,nlat_regional
       allocate(field2(nlon_regional,nlat_regional))
       write(iunit) iyear,imonth,iday,ihour,iminute,isecond, &
       ...
       call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
            ...
            ierr                                 )
       write(iunit) field2
       field2=field2/1000.0/9.8*3600.0*1000.0
       if(print_verbose)write(6,*)' max,min [mm/h] ',trim(rmse_var),'=',maxval(field2),minval(field2)
       ...
       call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
            ...
            ierr                                 )
       write(iunit) field2
       field2=field2/100.0*3600.0
       if(print_verbose)write(6,*)' max,min [hPa/h] ',trim(rmse_var),'=',maxval(field2),minval(field2)
  ...
  end subroutine convert_netcdf_constraints_wrf
