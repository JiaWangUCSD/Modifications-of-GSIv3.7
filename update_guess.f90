...
!$$$
  ...
  use constants, only: reflmin,hydromin
  use jfunc, only: iout_iter,bcoption,tsensible,clip_supersaturation
  use jfunc, only: wangjia_lmin_reflan
...
!    Update extra met-guess fields
     do ic=1,nguess
        ...
        if (id>0) then  ! Case when met_guess and state vars map one-to-one, take care of them together 
           ...
           if (trim(guess(ic))=='ql' .or. trim(guess(ic))=='qr' .or. trim(guess(ic))=='qi' .or. trim(guess(ic))=='qs' .or. trim(guess(ic))=='qg' .or. trim(guess(ic))=='qh') then
               call upd_positive_fldr3_(ptr3dges,ptr3dinc, hydromin)
               cycle
           endif
           ...
           if (trim(guess(ic))=='refl') then
               if(wangjia_lmin_reflan) then
                  call upd_positive_fldr3_(ptr3dges,ptr3dinc, reflmin)
               else
                  ptr3dges = ptr3dges + ptr3dinc
               end if
               cycle
