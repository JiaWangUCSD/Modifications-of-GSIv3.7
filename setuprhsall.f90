...
! Delcare local variables
  ...
  integer(i_kind) lunin,nobs,nchanl,nreal,nele,&
       is,idate,i_dw,i_rw,i_refl,i_sst,i_tcp,i_gps,i_uv,i_ps,i_lag,i_light,&
...
! wangjia: add "mype" as an argument to simplify the output
  if(.not.rhs_allocated) call rhs_alloc(mype,aworkdim2=size(awork1,2))
...
! wangjia:   type(obs_diags), pointer :: obsdiags(:,:) => null()  ! (nobs_type,nobs_bins)
...
!             Set up radar reflectivity
              else if(obstype=='refl')then
                 call setuprefl(lunin,mype,bwork,awork(1,i_refl),nele,nobs,is,conv_diagsave)
