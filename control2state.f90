...
integer(i_kind) :: icpblh,icgust,icvis,icoz,icwspd10m,icw,icrefl
...
character(len=4), parameter :: mycvars(ncvars) = (/  &  ! vars from CV needed here
                'sf  ', 'vp  ', 'ps  ', 't   ', 'q   ', 'cw  ', 'ql  ', 'qr  ', 'qi  ', 'qs  ', 'qg  ', 'qh  ', 'w   ', 'refl'/)
logical :: lc_sf,lc_vp,lc_w,lc_refl,lc_ps,lc_t,lc_rh,lc_cw,lc_ql,lc_qi
logical :: lc_qr,lc_qs,lc_qg,lc_qh
...
real(r_kind),pointer,dimension(:,:,:) :: cv_refl=>NULL()
...
real(r_kind),pointer,dimension(:,:,:) :: sv_refl=>NULL()
...
lc_ql  =icps(7)>0;  lc_qr  =icps(8)>0; 
lc_qi  =icps(9)>0;  lc_qs  =icps(10)>0;
lc_qg  =icps(11)>0; lc_qh  =icps(12)>0;
lc_w   =icps(13)>0; lc_refl=icps(14)>0
...
ls_ql  =isps(6)>0;   ls_qr  =isps(7)>0;
ls_qi  =isps(8)>0;   ls_qs  =isps(9)>0;
ls_qg  =isps(10)>0;  ls_qh  =isps(11)>0;
ls_w   =isps(12)>0;  ls_refl=isps(13)>0
...
call gsi_bundlegetpointer (xhat%step(1),'refl',icrefl,istatus)
...
!$omp section
   ...
   if (icrefl>0) then
      call gsi_bundlegetpointer (sval(jj),'refl' ,sv_refl, istatus)
      call gsi_bundlegetvar ( wbundle, 'refl', sv_refl, istatus )
   end if
   if (ictcamt>0) then 
