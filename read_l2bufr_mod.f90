! JiaWANG: modified GSI super-observation box, based on the area ( = del_range*del_range) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Get the index (1-based) in a radar volume
integer(i_kind) function getIndexInVolume_orgGSI(iaz, irr, iel, nazbin, nrbin, nelbin)

    use kinds, only: i_kind, r_kind
    implicit none
    integer(i_kind), intent(in) :: iaz    ! azimuthal bin index (1-based)
    integer(i_kind), intent(in) :: irr    ! radial index (1-based)
    integer(i_kind), intent(in) :: iel    ! elevation index (1-based)
    integer(i_kind), intent(in) :: nazbin, nrbin, nelbin

    getIndexInVolume_orgGSI = (iel-1_i_kind)*nazbin*nrbin+(irr-1_i_kind)*nazbin+iaz

end function getIndexInVolume_orgGSI


integer(i_kind) function getIndexInVolume_modGSI(iazrr, iel, nrbin, rdelr, nazbin_arr)

    use kinds, only: i_kind, r_kind
    implicit none
    integer(i_kind), intent(in) :: iazrr  ! index in a sweep (1-based)
    integer(i_kind), intent(in) :: iel    ! elevation index (1-based)
    integer(i_kind), intent(in) :: nrbin
    real(r_kind),    intent(in) :: rdelr
    integer(i_kind), dimension(nrbin), intent(in) :: nazbin_arr

    integer(i_kind) :: r
    real(r_kind) :: pi, c_now
    integer(i_kind) :: npt_per_sweep, nazbin_per_range

    pi = 3.14159265358979323846_r_kind

    !nrbin=nint(range_max/del_range)
    !delr=range_max/nrbin
    !rdelr =one/delr

    !npt_per_sweep = 0_i_kind
    !do r = 1, nrbin
    !   c_now = 2.0_r_kind * pi * (r*delr)
    !   nazbin_per_range = max(1, int(c_now / delr))
    !   npt_per_sweep = npt_per_sweep + nazbin_per_range
    !end do 
    !getIndexInVolume_modGSI = (iel-1_i_kind)*npt_per_sweep+iazrr

    getIndexInVolume_modGSI = getTotalElementsPerSweep_modGSI(nrbin,rdelr,nazbin_arr)*(iel-1_i_kind)+iazrr

end function getIndexInVolume_modGSI



integer(i_kind) function getIndexInSweep_modGSI(azm, range, nrbin, rdelr, nazbin_arr) ! iazrr

    use kinds, only: i_kind, r_kind
    use constants, only: one
    implicit none
    real(r_kind), intent(in)    :: azm, range
    integer(i_kind), intent(in) :: nrbin
    real(r_kind),    intent(in) :: rdelr
    integer(i_kind), dimension(nrbin), intent(in) :: nazbin_arr

    real(r_kind) :: pi, c_now, bin_width_deg, idx_now
    integer(i_kind) :: r, offset, nazbin_per_range, iaaa, irbin
    ...
    ! Make sure 1<=irbin<=nrbin
      irbin=ceiling(range*rdelr) ! >=1
      if (irbin > nrbin) irbin = nrbin ! just in case, range/delr=nrbin+0.00000001

    offset = 0_i_kind
    do r = 1, irbin-1
       !c_now = 2.0_r_kind * pi * (r*delr)
       !nazbin_per_range = max(1, int(c_now / delr))
       !nazbin_per_range = max(1, int(2.0_r_kind * pi * r))
       nazbin_per_range = nazbin_arr(r) 
       offset = offset + nazbin_per_range
    end do
    ...
    !nazbin_per_range = max(1, int(2.0_r_kind * pi * r))
    nazbin_per_range = nazbin_arr(r) 
    bin_width_deg = 360.0_r_kind / real(nazbin_per_range)
    idx_now = azm / bin_width_deg
    idx_now = modulo(idx_now, real(nazbin_per_range))       
    iaaa = int(floor(idx_now)) + 1
    offset = offset + iaaa

    getIndexInSweep_modGSI = offset

end function getIndexInSweep_modGSI



integer(i_kind) function getTotalElementsPerSweep_modGSI(nrbin, rdelr, nazbin_arr)
    ...
    getTotalElementsPerSweep_modGSI = npt_per_sweep

end function getTotalElementsPerSweep_modGSI



integer(i_kind) function getTotalElementsPerVolume_modGSI(nrbin, nelbin, rdelr, nazbin_arr)

    use kinds, only: i_kind, r_kind
    implicit none
    integer(i_kind), intent(in) :: nrbin, nelbin 
    real(r_kind),    intent(in) :: rdelr
    integer(i_kind), dimension(nrbin), intent(in) :: nazbin_arr
    ...
    getTotalElementsPerVolume_modGSI = getTotalElementsPerSweep_modGSI(nrbin,rdelr,nazbin_arr)*nelbin

end function getTotalElementsPerVolume_modGSI 
...
!$$$  end documentation block
    ...
    integer(i_kind) k,iii,j,jj,jjall,jjj,numzzzz,num_radars_0,iloc,iazrr
    integer(i_kind) iyref,imref,idref,ihref,iminref
    ...
    ! JiaWANG: nthisrad -> # super-obs a radar can have,  nthissweep -> # super-obs a sweep can have
    integer(i_kind) nthisrad,nthisbins,nthissweep
    integer(i_kind) idups,idups0
    integer(i_kind) nradials_in,nradials_fail_angmax,nradials_fail_time,nradials_fail_elb,nradials_fail_elb_pos,nradials_fail_elb_neg
    integer(i_kind) nradials_in1,nradials_fail_angmax1,nradials_fail_time1,nradials_fail_elb1,nradials_fail_elb1_neg,nradials_fail_elb1_pos
...
!            var  rbin*azbin*elbin rad#
    ...
    integer(i_kind),allocatable,dimension(:) :: nazbin_arr
    real(r_kind) :: pi
    ...
    allocate(nazbin_arr(nrbin))
    do i = 1, nrbin
      !nazbin_arr(i) = max(1, int(2.0_r_kind * pi * i))
       nazbin_arr(i) = max(1, int( (2.0_r_kind * i - 1.0_r_kind)* pi ))
    end do
...
!   Create unique master list of all radar names,lats,lons
    ...
    nradials_fail_elb=0
    nradials_fail_elb_pos=0
    nradials_fail_elb_neg=0
    nobs_badvr=0
    nobs_badsr=0
    nobs_lrbin=0
    nobs_hrbin=0
    nrange_max=0
    nthissweep = getTotalElementsPerSweep_modGSI(nrbin, rdelr, nazbin_arr) 
    nthisrad   = getTotalElementsPerVolume_modGSI(nrbin, nelbin, rdelr, nazbin_arr)
...
! reopen and reread the file for data this time
    ...
    do while(ireadmg(inbufr,subset,idate)>=0)
	  ...
	  stn_az=r90-hdr(8) ! JiaWang: ATTN
	  stn_el=hdr(9)
	  ielbin=ceiling(stn_el*rdelel)
	  if(ielbin<1.or.ielbin>nelbin) then
	     nradials_fail_elb=nradials_fail_elb+1
             if(ielbin<1)      nradials_fail_elb_neg=nradials_fail_elb_neg+1
             if(ielbin>nelbin) nradials_fail_elb_pos=nradials_fail_elb_pos+1
	     cycle
	  end if
	  stn_id=chdr 
	  ...
	  do i=1,n_gates
		...
		cycle
             ...
             iazrr = getIndexInSweep_modGSI(stn_az, range, nrbin, rdelr, nazbin_arr)
             if(iazrr<1 .or. iazrr>nthissweep) cycle
             iloc  = getIndexInVolume_modGSI(iazrr, ielbin, nrbin, rdelr, nazbin_arr)
             if(iloc<1 .or. iloc>nthisrad)     cycle
    ...
    call mpi_reduce(nradials_fail_elb,nradials_fail_elb1,1,&
	 mpi_integer4,mpi_sum,0,mpi_comm_world,ierror)
    call mpi_reduce(nradials_fail_elb_neg,nradials_fail_elb1_neg,1,&
	 mpi_integer4,mpi_sum,0,mpi_comm_world,ierror)
    call mpi_reduce(nradials_fail_elb_pos,nradials_fail_elb1_pos,1,&
	 mpi_integer4,mpi_sum,0,mpi_comm_world,ierror)
    ...
    if(mype==0 ) then
	  ...
	  if(rite)write(6,'(" master list radar ",i3," stn id,lat,lon,hgt,nSO = ",a4,2f10.2,f8.1,i10)') &
       ...
       if(rite)then
          ...
          write(6,*)' nthissweep=',nthissweep
          write(6,*)' nthisrad=',nthisrad
          write(6,*)' nthisbins=6*nthisrad',nthisbins
          write(6,*)' timemin,max [h]=',timemin1,timemax1
          ...
          ! nradials_fail_elb is after the maxmimum elevation check (nradials_fail_angmax), so nradials_fail_elb1_pos is always 0
          ! write(6,*)' nradials_fail_elb_all,neg,pos=',nradials_fail_elb1,nradials_fail_elb1_neg,nradials_fail_elb1_pos
          write(6,*)' nradials_fail_elb (mostly elv < 0.2-deg)=',nradials_fail_elb1
...
!    Print out histogram of counts by ielbin to see where angles are
       do ielbin=1,nelbin
          histo_el=sum(ibins2(getTotalElementsPerSweep_modGSI(nrbin, rdelr, nazbin_arr)*(ielbin-1)+1:getTotalElementsPerSweep_modGSI(nrbin, rdelr, nazbin_arr)*ielbin,1:num_radars_0))
	  if(rite)write(6,'(" elev-deg,histo_el=",f7.3,i20)') ielbin*delel,histo_el
