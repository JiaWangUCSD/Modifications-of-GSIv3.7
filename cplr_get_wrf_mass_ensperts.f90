...
! wangjia: nhr_assimilation could only be 2 for 3d EnVar, because it is related to "filelist02", which stores the name of ENS and 2 is from "nhr_assimilation"
      ...
      real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig):: u,v,tv,cwmr,oz,rh,w,refl,ql,qr,qi,qs,qg
...
! wangjia: ntlevs_ens->integer number of time levels for ensemble perturbations.  Default is 1, but will be equal to nobs_bins (4DVAR) when running in 4d-ensemble-var mode
      if(ntlevs_ens > 1) then
         ...
         ! wangjia: ntguessig->location of actual guess time for sigma fields
         its=ntguessig
         ite=ntguessig
      endif
  ...
  ! READ ENEMBLE MEMBERS DATA
            ...
            call this%general_read_wrf_mass(filename,ps,u,v,w,refl,ql,qr,qi,qs,qg,tv,rh,cwmr,oz,mype) 
...
! wangjia: modify the amplitude of moisture ("Q") & temperature ("T") perturbation
! NO change to the ensemble mean
  pert_percentage = 1.0_r_single
     do it=its,ite
        do n=1,n_ens
           do ic3=1,nc3d
              call gsi_bundlegetpointer(en_perts(n,it),trim(cvars3d(ic3)),w3,istatus)
              ...
              select case (trim(cvars3d(ic3)))
                  ...
                  case DEFAULT
                     do k=1,grd_ens%nsig
                        do i=1,grd_ens%lon2
                           do j=1,grd_ens%lat2
                              w3(j,i,k) = w3(j,i,k)*pert_percentage
                           end do
                        end do
                     end do
              end select
...
20 write(6,*) 'get_wrf_mass_ensperts_netcdf: read WRF-ARW ens failed ',n
  ...
  subroutine general_read_wrf_mass(this,filename,g_ps,g_u,g_v,g_w,g_refl,g_ql,g_qr,g_qi,g_qs,g_qg,g_tv,g_rh,g_cwmr,g_oz,mype)
  !$$$  subprogram documentation block
  !                .      .    .                                       .
  ...
  ! Declare passed variables
      ...
      real(r_kind),dimension(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig),intent(out):: &
                                                    g_u,g_v,g_w,g_refl,g_tv,g_rh,g_cwmr,g_oz, &
                                                    g_ql,g_qr,g_qi,g_qs,g_qg
  ...
  !   Declare local variables
      ...
      real(r_kind),allocatable,dimension(:,:,:):: tsn,qst,prsl,&
       gg_u,gg_v,gg_w,gg_refl,gg_tv,gg_rh,&
       gg_ql,gg_qr,gg_qi,gg_qs,gg_qg
  ...
  ! OPEN ENSEMBLE MEMBER DATA FILE
    if (mype==0) then ! only read data on root proc
      ...
      allocate(gg_refl(grd_ens%nlat,grd_ens%nlon,grd_ens%nsig))
      allocate(gg_ql(grd_ens%nlat,grd_ens%nlon,grd_ens%nsig))
  ...
  !
  ! INTERPOLATE TO MASS GRID
      !print *,'min/max w',minval(temp_3d),maxval(temp_3d),"   staggered"
      ...
      deallocate(temp_3d)
      deallocate(dim_id)
      print *,'min/max w',minval(gg_w),maxval(gg_w)," unstaggered"
  !
  ! READ reflectivity (dBZ)
      !print *, 'read REFL_10CM ',filename
      call nc_check( nf90_inq_varid(file_id,'REFL_10CM',var_id),&
          myname_,'inq_varid REFL_10CM '//trim(filename) )
 
      call nc_check( nf90_inquire_variable(file_id,var_id,ndims=ndim),&
          myname_,'inquire_variable REFL_10CM '//trim(filename) )
      allocate(dim_id(ndim))
      ...
      call nc_check( nf90_get_var(file_id,var_id,temp_3d),&
          myname_,'get_var REFL_10CM '//trim(filename) )
      gg_refl = reshape(temp_3d,(/dim(dim_id(2)),dim(dim_id(1)),dim(dim_id(3))/),order=(/2,1,3/))
      deallocate(temp_3d)
      deallocate(dim_id)
      print *,'min/max refl',minval(gg_refl),maxval(gg_refl)
  !
  ! READ QCLOUD (kg/kg)
      !print *, 'read QCLOUD ',filename
      call nc_check( nf90_inq_varid(file_id,'QCLOUD',var_id),&
          myname_,'inq_varid QCLOUD '//trim(filename) )

      call nc_check( nf90_inquire_variable(file_id,var_id,ndims=ndim),&
          myname_,'inquire_variable QCLOUD '//trim(filename) )
      allocate(dim_id(ndim))
      ...
      call nc_check( nf90_get_var(file_id,var_id,temp_3d),&
          myname_,'get_var QCLOUD '//trim(filename) )
      gg_ql = reshape(temp_3d,(/dim(dim_id(2)),dim(dim_id(1)),dim(dim_id(3))/),order=(/2,1,3/))
      deallocate(temp_3d)
      deallocate(dim_id)
      print *,'min/max ql  ',minval(gg_ql),maxval(gg_ql)
  !
  ! READ QRAIN (kg/kg)
  ...
  !
  ! READ QICE (kg/kg)
      !print *, 'read QICE ',filename
      call nc_check( nf90_inq_varid(file_id,'QICE',var_id),&
          myname_,'inq_varid QICE '//trim(filename) )

      call nc_check( nf90_inquire_variable(file_id,var_id,ndims=ndim),&
          myname_,'inquire_variable QICE '//trim(filename) )
      allocate(dim_id(ndim))
      ...
      call nc_check( nf90_get_var(file_id,var_id,temp_3d),&
          myname_,'get_var QICE '//trim(filename) )
  ...
  ! CALCULATE PSEUDO RELATIVE HUMIDITY IF USING RH VARIABLE
      if (.not.q_hyb_ens) then
         ...
         ! ice=T: include ice and ice-water effects, depending on t, in qsat calcuations.
  ...
  ! then TV,U,V,W,RH
    do k=1,grd_ens%nsig
       ...
       if (mype==0) call this%fill_regional_2d(gg_refl(1,1,k),wrk_fill_2d)
       call mpi_scatterv(wrk_fill_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype, &
       g_refl(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,0,mpi_comm_world,ierror)
       if (mype==0) call this%fill_regional_2d(gg_ql(1,1,k),wrk_fill_2d)
       call mpi_scatterv(wrk_fill_2d,grd_ens%ijn_s,grd_ens%displs_s,mpi_rtype, &
       g_ql(1,1,k),grd_ens%ijn_s(mype+1),mpi_rtype,0,mpi_comm_world,ierror)
  ...
  ! for now, don't do anything with oz, cwmr
    ...
    if (mype==0) deallocate(gg_u,gg_v,gg_w,gg_refl,gg_ql,gg_qr,gg_qi,gg_qs,gg_qg,gg_tv,gg_rh,gg_ps)
  ...
  !
    ...
    real(r_kind),pointer,dimension(:,:,:):: st,vp,w,refl,tv,rh,oz,cw
...
!                  if .false., ensemble perturbation wind stored as psi,chi.
    ...
    call gsi_bundlegetpointer(suba,'w',w,istat)
    if(istat/=0) then
       write(6,*)' no w  pointer in ens_spread_dualres, point w  at dum3 array'
       w  => dum3
    end if
    call gsi_bundlegetpointer(suba,'refl',refl,istat)
    if(istat/=0) then
       write(6,*)' no refl pointer in ens_spread_dualres, point refl at dum3 array'
       refl  => dum3
    end if
...
! wangjia: compared to "write_spread_dualres", "write_spread_dualres_wangjia" includes "w". Since "write_spread_dualres" is also used in "get_gefs_ensperts_dualres.f90", I add a similar subroutine "write_spread_dualres_wangjia"
    call write_spread_dualres_wangjia(st,vp,w,refl,tv,rh,oz,cw,ps,mype)
