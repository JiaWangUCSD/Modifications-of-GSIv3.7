module m_reflNode
! abstract: class-module of obs-type reflNode (radar radial winds)
...
! module interface:
  ...
  type,extends(obsNode):: reflNode
     !type(refl_ob_type),pointer :: llpoint => NULL()
     type(obs_diag), pointer :: diags => NULL()     
     real(r_kind)    :: res           !  reflectivity residual
     real(r_kind)    :: err2          !  reflectivity error squared
  ...
  public:: reflNode_typecast
  public:: reflNode_nextcast
        interface reflNode_typecast; module procedure typecast_ ; end interface
        interface reflNode_nextcast; module procedure nextcast_ ; end interface

  character(len=*),parameter:: MYNAME="m_reflNode"

#include "myassert.H"
#include "mytrace.H"
contains
function typecast_(aNode) result(ptr_)
!-- cast a class(obsNode) to a type(reflNode)
  use m_obsNode, only: obsNode
  implicit none
  type(reflNode),pointer:: ptr_
...
!-- cast an obsNode_next(obsNode) to a type(reflNode)
...
subroutine obsNode_xread_(aNode,iunit,istat,diagLookup,skip)
  ...
  implicit none
  class(reflNode),intent(inout):: aNode
...
subroutine obsNode_xwrite_(aNode,junit,jstat)
  implicit none
  class(reflNode),intent(in):: aNode
