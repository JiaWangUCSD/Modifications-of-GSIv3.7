...
! wangjia: add a check for reflectivity BUFR file
       else if(trim(filename) == 'radreflbufr')then  ! RTod: wired-in filename is not a good idea
         lexist = .false.
         file1loop: do while(ireadmg(lnbufr,subset,idate2) >= 0)
          do while(ireadsb(lnbufr)>=0)
           call ufbint(lnbufr,rtype,1,1,iret,'TYP')
           kx=nint(rtype)
           do nc=1,nconvtype
...
!wangjia: def njqc -  logical flag (T=Purser's nonlinear qc on, F=off). For now, NJQC=.false.
!         in the future, if NJQC=.true., I may need to add "converr_refl_read" and "convb_refl_read" here.
    if(njqc) then
...
!   type type of GPS data (if present)
    ...
    read_ears_rec1=0
    read_db_rec1=0
! wangjia: ndat-># of data types, which is # of lines in "OBS_INPUT" in comgsi_namelist.sh
    do i=1,ndat
       ...
       if (obstype == 't'  .or. obstype == 'uv' .or. &
          ...
          ! wangjia: only let mype=0 to write out
          if(mype==0) write(6,*)'READ_OBS:  ***ERROR*** - unknown ob type ',trim(obstype)
       end if
...
!   Loop over data files.  Each data file is read by a sub-communicator
    ...
    do ii=1,mmdat
       ...
       if (i > 0 .and. belong(i)) then
          ...
          twind=time_window(i)               !     time window (hours) for input group. time_window  - half time window for obs type (hours)
...
!            Process radar reflectivity Mosaic from https://rda.ucar.edu/datasets/ds841.0/
             else if (obstype == 'refl' ) then
! wangjia: nread-># of read; npuse-># of retained; nobs_sub1-># for PE
                call read_radarref_mosaic_gridrad(nread,npuse,nouse,infile,obstype,lunout,twind,sis, &
                      hgtl_full,nobs_sub1(1,i))
                string='READ_RADARREF_MOSAIC_GRIDRAD'
...
!            Process radar winds
             ...
             ! wangjia: for Radial Winds, there're two subroutines: read_radar & read_radar_l2rw_novadqc
             ! wangjia: vadwnd_l2rw_qc -> When false, DO NOT run the vadwnd qc on level 2 radial wind obs
             ! wangjia: there's a possibility that "vadwnd_l2rw_qc=.t. & sis=l2rw". In order to avoid it, I set "vadwnd_l2rw_qc=.f." WRONG!!!!!!!
             ! read_radar_l2rw_novadqc: does NOT thin the supob at all, could NOT use it. So I decide to modify "read_radar"
