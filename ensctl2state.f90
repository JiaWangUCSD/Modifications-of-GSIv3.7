...
real(r_kind),pointer,dimension(:,:,:) :: sv_u,sv_v,sv_w,sv_refl,sv_prse,sv_q,sv_tsen,sv_tv,sv_oz
...
! wangjia: xhat->constrol variable (static part), increments
!          sf/vp is used as control variables
...
! Since each internal vector of xhat has the same structure, pointers are
! the same independent of the subwindow jj
! wangjia: eval->state variables (eventually will be the whole part), increments
...
! wangjia:  q_hyb_ens:  if .true., then use specific humidity ensemble perturbations, if .false. (default), use relative humidity
!           for now, q_hyb_ens=False, which means, RH is used as control variable, and SH is used as state variables, so there needs a conversion
...
! wangjia: sf/vp is control variables, and u/v is state variables, so there needs a conversion
...
do_cw_to_hydro = lc_cw .and. ls_ql .and. ls_qi .and. (.not. lc_ql) .and. (.not. lc_qi)
do_cw_to_hydro_hwrf = .false.
do_cw_to_hydro_hwrf = lc_cw.and.ls_ql.and.ls_qi.and.ls_qr.and.ls_qs.and.ls_qg.and.ls_qh .and. (.not. lc_ql) .and. (.not. lc_qi) .and. (.not. lc_qr) .and. (.not. lc_qs) .and. (.not. lc_qg) .and. (.not. lc_qh)  
...
! wangjia: wbundle_c = sum{a_k x^e_k}. wbundle_c->control variable
   if(dual_res) then
...
!  Get pointers to required state variables
   ...
   call gsi_bundlegetpointer (eval(jj),'refl',sv_refl,istatus)
   call gsi_bundlegetpointer (eval(jj),'tsen',sv_tsen,istatus)


!  Get pointers to required state variables
!  Convert streamfunction and velocity potential to u,v
!  wangjia: uv_hyb_ens: if .true., then ensemble perturbation wind stored as u,v
!           for now, uv_hyb_ens=TRUE
   if(do_getuv) then
      if(uv_hyb_ens) then
...
! wangjia: add "W"
! wbundle_c: control variable (ensemble part)
! control variable to state variable
         call gsi_bundlegetvar ( wbundle_c, 'w', sv_w, istatus )

! wangjia: add "refl"
         call gsi_bundlegetvar ( wbundle_c, 'refl', sv_refl, istatus )
