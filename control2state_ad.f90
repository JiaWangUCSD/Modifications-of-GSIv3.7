...
character(len=4), parameter :: mycvars(ncvars) = (/  &
...
real(r_kind),pointer,dimension(:,:,:) :: rv_refl=>NULL()
...
call gsi_bundlegetpointer (grad%step(1),'refl',icrefl,istatus)
...
!  Same one-to-one map for chemistry-vars; take care of them together
   ...
   if (icrefl>0) then
      call gsi_bundlegetpointer (rval(jj),'refl' ,rv_refl, istatus)
      call gsi_bundleputvar ( wbundle, 'refl', rv_refl, istatus )
   end if
   if (ictcamt>0) then
